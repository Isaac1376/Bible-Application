...
function Books({ language }) {
  ...
  return <div className="space-y-6 sm:space-y-8">
    ...
    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{filteredBooks.map((book, index) => <motion.article key={book.id} initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.025, 0.35) }} whileHover={{ y: -5 }} className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-[#8d5722]/40 bg-[#140f09]/90 shadow-[0_0_40px_rgba(179,121,36,0.16)]"><img src={book.coverImage} alt="" className="h-36 w-full object-cover opacity-75 transition duration-500 group-hover:scale-105 group-hover:opacity-100" /><div className="flex flex-1 flex-col p-5"><div className="mb-3 flex items-center justify-between gap-3"><span className="rounded-full border border-[#d6a84f]/40 bg-[#24130a] px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-[#f0c66d]">{book.testament}</span><span className="flex items-center gap-1 text-xs text-[#d8bf86]"><Clock3 size={13} />{book.chapters} {isTamil ? 'அதி.' : 'ch.'}</span></div><h2 className="font-[Times_New_Roman,serif] text-2xl text-[#fff4d1]">{text(book.bookName[language])}</h2><p className="mt-2 text-sm leading-6 text-[#d6c09b]">{text(book.about[language])}</p><p className="mt-4 rounded-xl border border-[#6c4320]/50 bg-[#1b1209] p-3 text-sm italic leading-6 text-[#f5deb1]">“{book.importantVerses[0]}”</p><div className="mt-4 flex flex-wrap gap-2">{book.keyThemes.slice(0, 3).map((theme) => <span key={theme} className="rounded-full border border-[#6c4320]/50 bg-[#1d120b] px-2.5 py-1 text-xs text-[#f5deb1]">{theme}</span>)}</div><div className="mt-auto flex flex-wrap gap-x-4 gap-y-3 pt-5"><Link to={`/books/${book.id}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#ffd36b] hover:text-[#fff3c4]">{isTamil ? 'விவரங்கள்' : 'Study details'} <ArrowRight size={15} /></Link><a href={videoUrl(book, language)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#f0c66d] hover:text-[#fff3c4]"><PlayCircle size={16} />{isTamil ? 'கதை காணொளி' : 'Story video'}</a></div></div></motion.article>)}</div>
  </div>
}
